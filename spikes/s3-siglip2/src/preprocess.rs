...
//!   - create a 256x256 BGRA buffer, **fill black first** (recycled buffer memory;
...
/// Squash-resize `img` into a black-filled `IMAGE_SIZE`x`IMAGE_SIZE` RGBA square,
...
pub fn squash_to_square(img: &DynamicImage) -> RgbaImage {
    ...
    // model-card-specified resample and the closest portable match — see FINDINGS
    // "preprocessing parity" for the residual difference.)
...
/// Full preprocess: decode-agnostic `DynamicImage` -> ONNX `pixel_values`
/// `[3*256*256]` f32 in CHW order, values in [-1, 1].
///
...
mod tests {
    ...
    fn squash_produces_exact_square_no_crop() {
        // A wide 400x100 image must squash (not crop) to 256x256.
    ...
    fn mid_grey_maps_near_zero() {
        ...
        let grey = DynamicImage::ImageRgb8(RgbImage::from_pixel(8, 8, image::Rgb([128, 128, 128])));
