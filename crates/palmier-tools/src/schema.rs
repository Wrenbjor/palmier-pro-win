...
fn schema_set_clip_properties() -> Value {
    ...
    object_schema(
        &[
            ...
            ("transform", transform),
            ("rotation", num_prop("Static rotation in degrees (clockwise). Sets the transform's rotation; clears any existing rotation keyframes. Use set_keyframes for animated rotation.")),
            ("fadeInFrames", int_prop("Fade-in ramp length in frames (0 = no fade). Clamped to the clip duration.")),
            ("fadeOutFrames", int_prop("Fade-out ramp length in frames (0 = no fade). Clamped so fadeIn + fadeOut <= duration.")),
            ("fadeInInterpolation", enum_str_prop(&["linear", "hold", "smooth"], "Fade-in easing curve (default linear).")),
            ("fadeOutInterpolation", enum_str_prop(&["linear", "hold", "smooth"], "Fade-out easing curve (default linear).")),
