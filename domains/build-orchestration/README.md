...
2026-06-20 | E11-S4 VISUALINDEXER MERGED + green (943aadc) — sampler→embedder→store, idempotent needs_index (false on unreadable file, matching reference guard), first-shot-starts-0, shot_end=next-start/duration-for-last, still=single (0,0,0) row. Two trait abstractions: `FrameEmbedder` (encode step — blanket impl for the real VisualEmbedder under --features ort; mock in default tests so 68 tests run with no ort/weights) + `ExportYield` (export-pause hook, NoExportYield stub → E11-S6 injects the real ExportPauseCounter loop). 68 default tests + ort build green. **M4 visual indexing chain complete: sampler→embedder→store→indexer + ranking.** E11-S6 coordinator unblocked. E10-S7 still in flight.
2026-06-20 | E10-S7 CAPTION TOOLS MERGED + green (f480953) — add_captions + get_transcript bodies in palmier-tools. add_captions builds the transcribe closure for generate_captions: bypass=censor||locale (engine direct), else TranscriptCache (hit=range-filtered no-engine; miss=transcribe full→store→filter); blocking whisper via spawn_blocking; one verbatim "Generate Captions" agent-undo. get_transcript read-only (never transcribes; empty→agent must transcribe first), project-frame words, 10000-word paging. Tool count still EXACTLY 30, descriptions verbatim. **28/30 tools functional** (was 26; remaining: search_media=E11-S10, + generate live paths). 151 palmier-tools tests. **E10 only S8 (transcript-cut) remains** — DISPATCHED (palmier-tools, disjoint from E11-S6).
2026-06-20 | E11-S6 coordinator DISPATCHED (palmier-search integration hub, parallel to E10-S7/palmier-tools) — per-project queue + sequential worker + transcript/visual concurrency + 2s export-pause loop + fan-out + search entrypoint; defines a process-global ExportPauseCounter (palmier-export wiring FLAGGED as follow-up, NOT edited here); model-loader download() stays a stub (weights ungated). Default build tests scheduling/pause/fan-out via mocks; real encode under --features ort.
2026-06-20 | E11-S1 EMBEDDER PORT MERGED + green (1256f3a) — production SigLIP2 VisualEmbedder + model-loader state machine + Gemma tokenizer + ONNX manifest ported from the S-3 spike into real palmier-search. ort runtime FEATURE-GATED (`ort` off by default → default build needs no onnxruntime.dll; under --features ort = ONNX Runtime 2.0.0-rc.10 DirectML+CPU, load-dynamic). Explicit L2-normalize preserved (raw dot==cosine); modelVersion=2; constants agree with `spec` (no divergent dup). 58 default tests green + ort build type-checks. Live encode/cosine `#[ignore]`d, gated on ~750MB weights + onnxruntime.dll (E11-S6 hosts/packages; manifest SHA placeholders). Additive lib.rs/Cargo.toml union resolved (tokenizers + ort/ndarray optional + [features] alongside E11-S8's transcribe/unicode-normalization). **M4 visual pipeline primitives ALL in: embedder(S1)+store(S2)+sampler(S3)+ranking(S5)** → E11-S4 VisualIndexer unblocked. E10-S7 still in flight.
2026-06-20 | E10-S6 + E11-S8 MERGED + green (bed2238) — E10-S6 (captions integration: specs() + generate_captions() in palmier-edit [transcribe injected as a closure so palmier-edit stays whisper/tokio-free; 6 specs tests ported, "Generate Captions" undo; 102 tests] + the 2 engine seams wired in palmier-transcribe [resolve_locale_en + censor_result applied when flagged; 39 tests] + caption_theme/natural_size in palmier-text + CaptionsTab constants; pnpm green; Clip::timeline_frame verified already-correct, unchanged). E11-S8 (TranscriptSearch: all-terms case+diacritic-insensitive via unicode-normalization NFD, disk-only via E10-S4 cache, TranscriptHit≠visual Hit; sync, threads &TranscriptCache; 36 palmier-search tests). **E10 transcription: only S7 (tool bodies) + S8 (transcript-cut) remain.** E10-S7 DISPATCHED (palmier-tools, disjoint from the still-running E11-S1 embedder).
2026-06-20 | WAVE-4 dispatched (2 disjoint palmier-search workers, parallel to the still-running E10-S6) — **E11-S1 production embedder port** (port the S-3-proven SigLIP2 VisualEmbedder+model-loader+tokenizer+manifest from spikes/ into real palmier-search; ort feature-gated OFF-by-default, explicit L2-normalize, modelVersion=2; preprocess/tokenize/normalize tested without weights, live encode gated on ~750MB download) — closes the gap where E11-S1 was only spike-proven, not production. **E11-S8 transcript keyword search** (all-terms, case+diacritic-insensitive, disk-only via E10-S4 cache; SM-12 spoken = 100% recall). **Decision: E11-S7 (transcript read-cache) SUBSUMED by E10-S4** — no second cache; E11-S8 reads `palmier_transcribe::TranscriptCache`. Both edit palmier-search (different files: embedder modules vs transcript_search.rs) — additive merge; distinct hit type (SpokenHit vs E11-S5 Hit).
2026-06-20 | E11-S5 + E11-S3 MERGED + green (post-d38c4a2) — S5 (VisualSearch.rs: raw-dot ranking on pre-normalized vecs [plain Rust dot, no BLAS — encoder is the cost], best-per-shot dedupe on shot_start.to_bits(), reference sort→drop<0.05→top>0→prefix(20)→filter≥top*0.85; min_score kept Option<f32> for parity; 8 tests), S3 (sampler.rs: 8×8 BT.601 luma-grid shot detection, 2s/8s cadence, ≥3000px edge doubles interval, samplerVersion=1; + palmier-media `extract_frame_timed`/`TimedFrame` surfacing true PTS so the "skip actualTime≤prev" near-dup-keyframe collapse actually works; 3 accept tests + ffmpeg integ behind PALMIER_TEST_VIDEO). palmier-search 29 tests, palmier-media 89, both green. lib.rs auto-merged. **M4 search primitives in: .embed store (S2) + FrameSampler (S3) + VisualSearch ranking (S5)** — feed E11-S4 (VisualIndexer) + E11-S6 (coordinator). E10-S6 still in flight.
2026-06-20 | WAVE-3 dispatched (3 disjoint workers) — E10-S6 (captions integration: specs() mapping + generateCaptions orchestration + caption-track placement in palmier-edit + media-panel; ALSO ports the 6 deferred specs tests + wires the 2 engine seams [resolve_locale_en, censor_result]), E11-S3 (palmier-search FrameSampler.rs: 8×8 BT.601 luma-grid shot detection + 2s/8s cadence, samplerVersion=1), E11-S5 (palmier-search VisualSearch.rs: raw-dot ranking on pre-normalized vecs, best-per-shot dedupe, 0.05 floor/0.85 cutoff). E10-S6 solo (palmier-edit/media-panel/transcribe-seam); E11-S3+S5 share palmier-search but different files (additive merge). E10-S6 unblocks E10-S7→S8; E11-S3/S5 feed E11-S4/S6.
2026-06-20 | E10-S2 ENGINE MERGED + green (0837a37) — FFmpeg 16kHz/mono/s16le **forced** extraction (solved libswresample AVERROR_INPUT_CHANGED by stamping each frame's channel_layout + reinit-retry) + whisper-rs 0.16 run; **LIVE ggml-small.en transcription proven** (JFK sample → correct text, 5.49s CPU, lang=en). Sync API (whisper `full()` blocks → E10-S6 uses spawn_blocking). Additive Cargo.toml/lib.rs union resolved (whisper-rs/ffmpeg-next/uuid added; dropped S2's dup dirs + stale dev serde_json). palmier-transcribe now **38 tests**; full workspace 52 suites green. ⚑ **2 integration seams open for E10-S6:** (a) engine still uses a private `resolve_locale_stub→"en"` (correct for the bundled .en model) at `// E10-S3 SEAM` markers — wire to the merged `resolve_locale_en`; (b) `censor_profanity` is threaded but NOT applied — wire to merged `censor_result`. **E10 transcription core (S1–S5) COMPLETE.**
2026-06-20 | E10-S4 + E10-S3 MERGED + green (8d39311) — S4 (TranscriptCache: sha256(first-16MiB+len+model+lang) key #19 [chose 16 MiB prefix + length-mixed], windowed-filter returns subset w/o re-transcribe, 4-entry clear-all, tokio OnceCell singleton; 7 tests), S3 (locale resolver: exact-region-pref matchLocale parity + sys-locale OS auto-detect; profanity bracketed censoring; 22 tests). palmier-transcribe now 33 tests green (S1+S3+S4). Additive Cargo.toml union resolved (serde_json promoted + sha2/dirs/tokio + sys-locale); lib.rs auto-merged. ⚑ **Parity caveat:** profanity word/replacement list is **clean-room seed** — Apple's `.etiquetteReplacements` set lives in a closed framework, unobservable; overridable via `censor_*_with`. E10-S2 (engine) still in flight.
